...
export default function DistributionVerification({ equipment, assignments, onRefresh }: DistributionProps) {
  ...
  const handleCheckout = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response = await apiFetch('/api/assignments/checkout', {
        method: 'POST',
  ...
  const handleReclaim = async (assignmentId: number, equipmentName: string) => {
    ...
    try {
      const response = await apiFetch(`/api/assignments/reclaim/${assignmentId}`, {
        method: 'POST'
      });
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-955 border border-slate-800 rounded-lg pl-9 pr-3 py-2 text-sm text-slate-202 focus:outline-none focus:border-indigo-500 [color-scheme:dark] hover:border-slate-755 transition-colors"
                      placeholder="No decided deadline"
                    />
                  </div>
                </div>
              </div>
                                              ...
                                              <div key={asg.assignment_id} className="flex items-center justify-between bg-slate-900/80 border border-slate-850 p-2 rounded-lg hover:border-slate-800 transition-colors">
                                                <div className="space-y-0.5 min-w-0 pr-2">
                                                  ...
                                                  <div className="text-[9px] text-slate-500 font-mono">
                                                    Out since {asg.checkout_date}{asg.until_date ? ` ➔ Until ${asg.until_date}` : ''}
                                                  </div>
                                                </div>
                                                <button
                                                  type="button"
                                ...
                                {modelAssignments.map(asg => (
                                  <div key={asg.assignment_id} className="bg-slate-900/80 border border-slate-850 p-3 rounded-lg flex flex-col justify-between gap-3 hover:border-slate-800 transition-colors text-xs">
                                    <div className="space-y-1.5 min-w-0">
                                      ...
                                      <div className="text-[9px] text-slate-500 font-mono">
                                        Issued date: {asg.checkout_date}{asg.until_date ? ` ➔ Until ${asg.until_date}` : ''}
                                      </div>
                                    </div>
