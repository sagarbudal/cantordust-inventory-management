...
import { 
  ...
  Lock, Shield, Video, Layers, Tag, User
} from 'lucide-react';
...
export default function VideoManager({ videos, onRefresh }: VideoManagerProps) {
  ...
  const [duplicatePromptSingle, setDuplicatePromptSingle] = useState(false);
  const [bulkDefaultStatus, setBulkDefaultStatus] = useState<'uploaded' | 'not uploaded'>('uploaded');
  ...
  const [manualFactoryCode, setManualFactoryCode] = useState('');
  ...
  // Category management & compulsory selection states
  const [categoriesList, setCategoriesList] = useState<string[]>([]);
  const [manualVideoCategory, setManualVideoCategory] = useState('Marketing');
  const [bulkVideoCategory, setBulkVideoCategory] = useState('Marketing');

  // Operator and recorded date states
  ...
  const [manualRecordedDate, setManualRecordedDate] = useState(() => new Date().toISOString().split('T')[0]);
  const [bulkRecordedDate, setBulkRecordedDate] = useState(() => new Date().toISOString().split('T')[0]);
  ...
  const fetchCategoriesList = async () => {
    try {
      ...
      if (res.ok) {
        ...
        if (data.length > 0) {
          setManualVideoCategory(prev => prev && data.includes(prev) ? prev : data[0]);
          setBulkVideoCategory(prev => prev && data.includes(prev) ? prev : data[0]);
        }
      }
    } catch (err) {
  ...
  const fetchOperatorsList = async () => {
    try {
      ...
      if (res.ok) {
        ...
        if (data.length > 0) {
          setManualOperatorName(prev => prev && data.includes(prev) ? prev : data[0]);
          setBulkOperatorName(prev => prev && data.includes(prev) ? prev : data[0]);
        }
      }
    } catch (err) {
  ...
  const parseVideoFile = (file: File): Promise<StagedVideo> => {
    return new Promise((resolve) => {
      ...
      const fallback: StagedVideo = {
        ...
        fileName: file.name,
        sub_category: '',
        video_category: bulkVideoCategory || 'Marketing',
        operator_name: bulkOperatorName || (operatorsList && operatorsList[0] ? operatorsList[0] : 'N/A'),
        recorded_date: bulkRecordedDate || new Date().toISOString().split('T')[0],
        isDuplicate,
    ...
    try {
      const res = await apiFetch('/api/custom-folders', {
        ...
        body: JSON.stringify({
          ...
          factory_code: manualFactoryCode.trim()
        })
      });
      const data = await res.json();
      if (!res.ok) {
  ...
  const handleDeleteCustomFolder = async (id: number) => {
    ...
    setFolderSuccess(null);
    try {
      const res = await apiFetch(`/api/custom-folders/${id}`, { method: 'DELETE' });
      const data = await res.json();
  ...
  const handlePurgeFolder = async (folderName: string, subName?: string) => {
    const label = subName ? `sub-folder "${subName}" inside "${folderName}"` : `folder "${folderName}"`;
    const confirmPrompt = `WARNING: Are you sure you want to permanently delete ${label} along with ALL of its files and nested sub-folders? This is 100% permanent and irreversible!`;
    ...
    try {
      const res = await apiFetch('/api/custom-folders/purge', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ category: folderName, sub_category: subName })
      });

      const data = await res.json();
      if (!res.ok) {
        throw new Error(data.error || 'Failed to purge folder data.');
      }

      setSuccessMsg(`✓ Successfully deleted ${label} and permanently deleted all corresponding videos.`);
      
      // Refetch both videos and custom-folders configurations
      fetchCustomFolders();
      onRefresh();
    } catch (err: any) {
      setErrorMsg(err.message || 'Error occurred while purging directory.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (hasDupCode && !duplicatePromptSingle) {
      setDuplicatePromptSingle(true);
      return;
    }
    ...
    try {
      ...
      const res = await apiFetch('/api/videos', {
        ...
        body: JSON.stringify({
          ...
          video_category: manualVideoCategory || 'Marketing',
          operator_name: manualOperatorName,
          recorded_date: manualRecordedDate
        })
      });
      const data = await res.json();
      ...
      setCustomFolder('');
      setDuplicatePromptSingle(false);
      onRefresh();
    } catch (err: any) {
      ...
      for (let i = 0; i < stagedVideos.length; i++) {
        ...
        const res = await apiFetch('/api/videos', {
          ...
          body: JSON.stringify({
            ...
            video_category: item.video_category || bulkVideoCategory || 'Marketing',
            operator_name: item.operator_name || bulkOperatorName || (operatorsList && operatorsList[0] ? operatorsList[0] : 'N/A'),
            recorded_date: item.recorded_date || bulkRecordedDate || new Date().toISOString().split('T')[0]
          })
        });
  ...
  const filteredVideos = videos.filter(v => {
    ...
    const videoCat = v.category || 'Uploaded Files';
    const matchesCategory = selectedCategory === 'All' || videoCat.toLowerCase() === selectedCategory.toLowerCase();
  ...
  const groupedFolderNames = Object.keys(foldersGrouped)
    .filter(fn => {
      const folderVideos = foldersGrouped[fn];
      if (hasActiveFilters) {
        return folderVideos.length > 0;
      }
      return folderVideos.length > 0 || customFoldersList.some(cf => cf.category === fn);
    })
    .sort((a, b) => a.localeCompare(b));
    ...
    return (
      <div className="rounded-xl overflow-hidden border border-slate-800 bg-slate-950/20 shadow-lg text-xs">
        ...
        <div className="bg-slate-900 border-b border-slate-800 px-3 py-2 flex items-center gap-2 text-[10px] font-mono text-slate-400 select-none">
          <div className="bg-slate-950 px-2 py-0.5 rounded text-indigo-400 font-bold border border-slate-800/80 shrink-0">
            CANTOR_DUST_SHEET
          </div>
          ...
          <div className="bg-slate-950/50 border border-slate-800/60 rounded px-2.5 py-0.5 flex-1 text-slate-350 truncate">
            {vids.length === 0 ? "EMPTY_SHEET" : `=SUM(E2:E${vids.length + 1}) | ${vids.length} entries matching current view`}
          </div>
        </div>
          ...
          <table className="min-w-full border-collapse text-left text-xs text-slate-200">
            ...
            <thead className="bg-slate-900/90 text-[10px] text-slate-400 font-mono select-none">
              <tr className="divide-x divide-slate-800 border-b border-slate-800">
                ...
                <th className="px-3 py-1 text-center bg-slate-950/60 w-12">A</th>
                <th className="px-3 py-1 text-center bg-slate-950/60 w-16">B</th>
                <th className="px-3 py-1 text-center bg-slate-950/60">C</th>
                <th className="px-3 py-1 text-center bg-slate-950/60 w-32">D</th>
                <th className="px-3 py-1 text-center bg-slate-950/60 w-28">E</th>
                <th className="px-3 py-1 text-center bg-slate-950/60 w-28 hidden md:table-cell">F</th>
                <th className="px-3 py-1 text-center bg-slate-950/60 w-28">G</th>
                <th className="px-3 py-1 text-center bg-slate-950/60 w-28">H</th>
                <th className="px-3 py-1 text-center bg-slate-950/60 w-28">I</th>
              </tr>
              <tr className="bg-slate-950/70 border-b border-slate-800 text-[10px] md:text-[11px] text-slate-300 font-bold uppercase tracking-wider divide-x divide-slate-800/80">
                ...
                <th scope="col" className="px-3 py-2 text-center">
                  <input
                    ...
                    className="rounded border-slate-750 bg-slate-950 text-indigo-600 focus:ring-0 focus:ring-offset-0 cursor-pointer h-3.5 w-3.5"
                  />
                </th>
                ...
                <th scope="col" className="px-3 py-2 text-right font-mono">Duration</th>
                <th scope="col" className="px-3 py-2 hidden md:table-cell text-center font-mono text-slate-400">Recorded Date</th>
                <th scope="col" className="px-3 py-2 text-center font-mono text-slate-400">Operator</th>
            ...
            <tbody className="divide-y divide-slate-800/60">
              {vids.map((vid, idx) => {
                ...
                return (
                  <tr 
                    key={vid.id} 
                    className={`${isSelected ? 'bg-indigo-950/30' : 'hover:bg-slate-850/20 bg-slate-950/5'} transition-all divide-x divide-slate-800/60 border-b border-slate-800/60 text-slate-100 font-sans`}
                  >
                    ...
                    <td className="px-3 py-2 text-center whitespace-nowrap">
                      <input
                        ...
                        className="rounded border-slate-755 bg-slate-950 text-indigo-600 focus:ring-0 focus:ring-offset-0 cursor-pointer h-3.5 w-3.5"
                      />
                    </td>

                    {/* ID */}
                    <td className="px-3 py-2 font-mono font-bold text-slate-400 text-center text-[11px] whitespace-nowrap">{vid.id}</td>

                    {/* Video Title */}
                    <td className="px-3 py-2 font-medium text-slate-200 text-xs">{vid.name}</td>

                    {/* Code */}
                    <td className="px-3 py-2 font-mono text-slate-350 text-center text-[10px] whitespace-nowrap">
                      <span className="bg-slate-950 border border-slate-800 px-2 py-0.5 rounded uppercase tracking-wide">
                        {vid.unique_code}
                      </span>
                    </td>

                    {/* Duration */}
                    <td className="px-3 py-2 font-mono text-slate-300 text-right font-bold text-[11px] whitespace-nowrap">
                      {formatHHMMSS(vid.duration)}
                    </td>

                    {/* Recorded Date */}
                    <td className="px-3 py-2 text-indigo-350 font-mono text-[10px] text-center hidden md:table-cell whitespace-nowrap">
                      {vid.recorded_date || 'N/A'}
                    </td>

                    {/* Operator Name */}
                    <td className="px-3 py-2 text-slate-300 font-sans text-xs text-center whitespace-nowrap">
            ...
            {/* Sheet Summary Row */}
            <tfoot className="bg-slate-950/70 border-t border-slate-800 text-[10px] md:text-xs">
              <tr className="divide-x divide-slate-800">
                <td className="w-10 bg-slate-950/90 text-slate-600 border-r border-slate-850"></td>
                <td colSpan={3} className="px-3 py-2.5 text-slate-400 font-medium">
                  Total Rows: <strong className="text-slate-200 font-mono font-bold">{vids.length}</strong>
                </td>
                <td colSpan={5} className="px-3 py-2.5 text-right font-mono text-slate-400">
                  SUM Segment Duration: <strong className="text-indigo-400 font-black">{formatHHMMSS(tableTotalDuration)}</strong>
                </td>
              </tr>
            </tfoot>
          </table>
    ...
    <div className="space-y-6" id="video-manager-section">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl flex flex-col min-h-[500px] lg:min-h-[750px] h-auto transition-all duration-300">
          ...
          {stagedVideos.length > 0 ? (
            <form onSubmit={handleBulkSubmit} className="flex-1 flex flex-col overflow-hidden min-h-0 space-y-4">
              <div className="bg-slate-950/40 border border-slate-800 p-3.5 rounded-xl space-y-3.5 shrink-0">
                ...
                <div className="pt-2.5 border-t border-slate-850 space-y-2">
                  ...
                  <div className="pt-2.5 border-t border-slate-850 space-y-1.5 animate-fade-in">
                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        const newStat = e.target.value as 'uploaded' | 'not uploaded';
                        setBulkDefaultStatus(newStat);
                        setStagedVideos(prev => prev.map(item => ({ ...item, status: newStat })));
                      }}
                      className="w-full bg-slate-950 border border-slate-850 rounded px-2.5 py-1.5 text-xs text-slate-200 focus:outline-none focus:border-indigo-500 cursor-pointer"
                    >
                  ...
                  <div className="pt-2.5 border-t border-slate-850 space-y-1.5 animate-fade-in">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        setBulkVideoCategory(val);
                        setStagedVideos(prev => prev.map(item => ({ ...item, video_category: val })));
                      }}
                      className="w-full bg-slate-950 border border-slate-850 rounded px-2.5 py-1.5 text-xs text-slate-201 focus:outline-none focus:border-indigo-500 cursor-pointer text-indigo-100"
                      required
                    >
                      <option value="" disabled>-- Select Compulsory Tag --</option>
                  ...
                  {/* Compulsory Operator Selection */}
                  <div className="pt-2.5 border-t border-slate-850 space-y-1.5 animate-fade-in">
                    <label className="block text-[10px] font-bold text-indigo-400 uppercase tracking-wider flex items-center gap-1.5 font-mono">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        const val = e.target.value;
                        setBulkOperatorName(val);
                        setStagedVideos(prev => prev.map(item => ({ ...item, operator_name: val })));
                      }}
                      className="w-full bg-slate-950 border border-slate-850 rounded px-2.5 py-1.5 text-xs text-slate-201 focus:outline-none focus:border-indigo-500 cursor-pointer text-indigo-100"
                      required
                    >
                  ...
                  {/* Recorded Date Selection */}
                  <div className="pt-2.5 border-t border-slate-850 space-y-1.5 animate-fade-in">
                    <label className="block text-[10px] font-bold text-indigo-400 uppercase tracking-wider flex items-center gap-1.5 font-mono">
                      <Calendar className="h-3 w-3 scale-110" />
                      Recorded Date <span className="text-red-500">*</span>
                    </label>
                    <input
                      type="date"
                      value={bulkRecordedDate}
                      onChange={(e) => {
                        const val = e.target.value;
                        setBulkRecordedDate(val);
                        setStagedVideos(prev => prev.map(item => ({ ...item, recorded_date: val })));
                      }}
                      className="w-full bg-slate-950 border border-slate-850 rounded px-2.5 py-1.5 text-xs text-indigo-100 focus:outline-none focus:border-indigo-500 cursor-pointer"
                      required
                    />
                  </div>
              ...
              <div className="max-h-[170px] overflow-y-auto space-y-3 pr-1 min-h-[100px] bg-slate-950/35 p-2.5 rounded-xl border border-slate-850">
                {stagedVideos.map((item, index) => (
                  <div key={item.id} className="relative bg-slate-950/50 border border-slate-850 hover:border-slate-800 rounded-xl p-3 space-y-2 transition-colors">
                    ...
                    <div className="space-y-2 text-xs">
                      ...
                      <div className="grid grid-cols-2 gap-1.5">
                        <div>
                          <label className="block text-[9px] uppercase font-bold tracking-wider text-slate-500 mb-0.5 font-mono">Operator *</label>
                          <select
                            value={item.operator_name || ''}
                            onChange={(e) => updateStagedItem(item.id, { operator_name: e.target.value })}
                            className="w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded px-2 py-0.5 text-xs text-slate-300 focus:outline-none cursor-pointer"
                            required
                          >
                            {operatorsList.map(op => (
                              <option key={op} value={op}>{op}</option>
                        ...
                        <div>
                          <label className="block text-[9px] uppercase font-bold tracking-wider text-slate-500 mb-0.5 font-mono">Rec. Date *</label>
                          <input
                            type="date"
                            value={item.recorded_date || ''}
                            onChange={(e) => updateStagedItem(item.id, { recorded_date: e.target.value })}
                            className="w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded px-2 py-0.5 text-xs text-slate-350 focus:outline-none cursor-pointer"
                            required
                          />
                        </div>
                      </div>
            ...
            /* CONDITIONAL FORM OR DRAG PROMPT */
            !showManualForm ? (
              <div className="flex-1 flex flex-col justify-center items-center py-6 text-center animate-fade-in">
                {errorMsg && (
                  <div className="p-2 bg-red-955/15 border border-red-900/30 rounded text-[10px] text-red-405 flex items-center justify-between gap-1 w-full max-w-[260px] mb-3">
                    <span className="truncate">{errorMsg}</span>
                    <button type="button" onClick={() => setErrorMsg(null)} className="font-bold text-slate-400 hover:text-white px-1 font-mono text-xs cursor-pointer select-none">✕</button>
                  </div>
                )}
                {successMsg && (
                  <div className="p-2 bg-emerald-955/15 border border-emerald-900/30 rounded text-[10px] text-emerald-405 flex items-center justify-between gap-1 w-full max-w-[260px] mb-3">
                    <span className="truncate">{successMsg}</span>
                    <button type="button" onClick={() => setSuccessMsg(null)} className="font-bold text-slate-400 hover:text-white px-1 font-mono text-xs cursor-pointer select-none">✕</button>
                  </div>
                )}
                <p className="text-[11px] text-slate-450 leading-relaxed font-medium">
                  Drop local video or audio files in the dash area above to queue, extract metadata, and catalog in batch.
                </p>
                <div className="w-10 h-[1px] bg-slate-800 my-4 shrink-0" />
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="px-4 py-2 bg-slate-950 hover:bg-slate-850 hover:text-white border border-slate-800 hover:border-slate-705 text-slate-400 text-[10px] font-bold rounded-lg transition-all uppercase tracking-wider cursor-pointer"
                >
                  Or catalog a video manually
                </button>
              </div>
            ) : (
              <div className="flex-1 flex flex-col min-h-0 overflow-hidden animate-slide-up">
                <div className="flex items-center justify-between mb-3 shrink-0 border-b border-slate-850 pb-2">
                  <span className="text-[10px] font-black uppercase text-indigo-400 tracking-wider">Manual Metadata Entry</span>
                  <button
                    type="button"
                    onClick={() => {
                    ...
                    className="text-[9px] text-slate-455 text-slate-400 hover:text-white font-bold uppercase cursor-pointer"
                  >
                    ✕ Back to Drag
                  </button>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4 flex-1 overflow-y-auto pr-1">
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-slate-400 mb-1.5 uppercase tracking-wider">
                      Unique Code / Suffix ID
                    </label>
                    <input
                      type="text"
                      required
                      className="w-full bg-slate-950 border border-slate-800 rounded-lg px-2.5 py-1.5 text-xs font-mono text-slate-200 uppercase focus:outline-none focus:border-indigo-500"
                      value={uniqueCode}
                      onChange={(e) => {
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-slate-955 border border-slate-800 rounded-lg px-2.5 py-1.5 text-xs text-slate-202 focus:outline-none focus:border-indigo-505"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] font-bold text-slate-400 mb-1.5 uppercase tracking-wider">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-slate-950 border border-slate-800 rounded-lg px-2.5 py-1.5 text-xs text-slate-200 focus:outline-none focus:border-indigo-505"
                    />
                  </div>

                  <div>
                    <label className="block text-[10px] font-bold text-indigo-400 mb-1.5 uppercase tracking-wider flex items-center gap-1">
                      Video Category <span className="text-red-500 font-bold">*</span>
                    </label>
                    <select
                      value={manualVideoCategory}
                      onChange={(e) => setManualVideoCategory(e.target.value)}
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-indigo-400 mb-1.5 uppercase tracking-wider flex items-center gap-1 font-mono">
                      Operator Name <span className="text-red-500 font-bold">*</span>
                    </label>
                    <select
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-indigo-400 mb-1.5 uppercase tracking-wider flex items-center gap-1 font-mono">
                      Recorded Date <span className="text-red-500 font-bold">*</span>
                    </label>
                    <input
                      type="date"
                      value={manualRecordedDate}
                      onChange={(e) => setManualRecordedDate(e.target.value)}
                      className="w-full bg-slate-950 border border-slate-800 rounded-lg px-2.5 py-1.5 text-xs text-slate-202 focus:outline-none"
                      required
                    />
                  </div>

                  {duplicatePromptSingle && (
                    <div className="p-3 bg-amber-955/30 border border-amber-800/40 rounded-lg space-y-2 text-xs">
                      <p className="font-bold text-amber-500 flex items-center gap-1 font-mono uppercase text-[9px]">
                        ⚠️ Target Identifier code already exists
                      </p>
                      <p className="text-slate-400 text-[10px] leading-relaxed">
                        Do you want to skip registering this duplicate code or continue with a custom suffix?
                      </p>
                      <div className="grid grid-cols-2 gap-2 text-[9px] uppercase font-bold text-center">
                        <button
                          type="button"
                          onClick={() => {
                            setName('');
                            setUniqueCode('');
                            setDuplicatePromptSingle(false);
                            setSuccessMsg('✓ Skipped duplicate code registration.');
                          }}
                          className="py-1 bg-slate-950 border border-slate-800 hover:border-slate-700 text-slate-300 rounded cursor-pointer transition-all"
                        >
                          Skip Registration
                        </button>
                        <button
                          type="button"
                          onClick={async () => {
                            const sameBasenameCount = videos.filter(v => v.unique_code.toUpperCase().startsWith(uniqueCode.trim().toUpperCase())).length;
                            const suffixedCode = `${uniqueCode.trim().toUpperCase()}-DUP-${sameBasenameCount + 1}`;
                            const folderToSave = (isCreatingNewFolder ? customFolder : category)?.trim();
                            setLoading(true);
                            try {
                              const res = await apiFetch('/api/videos', {
                              ...
                              setSuccessMsg(`✓ Created (duplicate suffixed): "${data.name}"`);
                              setName('');
                              setUniqueCode('');
                              setDuration(5.0);
                              setSubCategory('');
                          ...
                          className="py-1 bg-amber-600/20 border border-amber-500 hover:bg-amber-600 text-amber-300 hover:text-white rounded cursor-pointer transition-all"
                        >
                  ...
                  {errorMsg && (
                    <div className="p-2 bg-red-955/15 border border-red-900/30 rounded text-[10px] text-red-405 flex items-center justify-between w-full">
                      <span>{errorMsg}</span>
                      <button type="button" onClick={() => setErrorMsg(null)} className="font-bold text-slate-400 hover:text-white px-1 cursor-pointer font-mono text-xs">✕</button>
                    </div>
                  )}
                  {successMsg && (
                    <div className="p-2 bg-emerald-955/15 border border-emerald-900/30 rounded text-[10px] text-emerald-405 flex items-center justify-between w-full">
                      <span>{successMsg}</span>
                      <button type="button" onClick={() => setSuccessMsg(null)} className="font-bold text-slate-400 hover:text-white px-1 cursor-pointer font-mono text-xs">✕</button>
                    </div>
                  )}
                  ...
                  {!duplicatePromptSingle && (
                    <button
                      type="submit"
                      disabled={loading}
                      className="w-full py-2 bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-800 disabled:text-slate-500 disabled:cursor-not-allowed text-white font-bold rounded-lg text-xs uppercase cursor-pointer"
                    >
                      {loading ? 'Registering...' : 'Catalog Video Unit'}
                    </button>
                  )}
                </form>
              </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl space-y-4">
            ...
            <form onSubmit={handleCreateCustomFolder} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1 px-1 font-mono">
                  Factory Code <span className="text-indigo-400">*</span>
                </label>
                <input
                  type="text"
                  required
                  value={manualFactoryCode}
                  placeholder="e.g. SL-V100, FACTORY_WEST"
                  onChange={(e) => setManualFactoryCode(e.target.value)}
                  className="w-full bg-slate-955 border border-slate-805 rounded-lg px-2.5 py-1.5 text-xs text-slate-205 focus:outline-none"
                />
              </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        {canDelete && (
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              handlePurgeFolder(folderName);
                            }}
                            className="inline-flex items-center gap-1.5 bg-rose-955/35 hover:bg-rose-900/20 border border-rose-900/40 text-[10px] uppercase font-bold tracking-wider px-2 py-1 text-rose-400 rounded cursor-pointer transition-colors animate-fade-in"
                            title="Permanently Purge Folder and All Contents"
                          >
                            <Trash2 className="h-3.5 w-3.5" />
                            Delete Permanent
                          </button>
                                ...
                                <div className="flex items-center gap-2.5">
                                  ...
                                  {canDelete && (
                                    <button
                                      ...
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        handlePurgeFolder(folderName, subName);
                                      }}
                                      className="inline-flex items-center gap-1 bg-rose-955/35 hover:bg-rose-900/20 border border-rose-900/40 text-[10px] uppercase font-bold tracking-wider rounded py-0.5 px-2 text-rose-450 text-rose-400 cursor-pointer transition-all shrink-0 font-sans"
                                      title={`Permanently delete the sub-folder "${subName}" and all of its files`}
                                    >
