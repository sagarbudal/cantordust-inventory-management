...
export default function UserAccessPanel({ onRefreshData }: UserAccessPanelProps) {
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    if (!emailInput || !password.trim()) {
      setErrorMsg('Email and Password are required.');
      return;
    }

    if (!emailInput.endsWith('@gmail.com')) {
      setErrorMsg('Access is strictly restricted to original Gmail accounts (@gmail.com). Please register a valid Gmail address.');
      return;
    }
    ...
    try {
      ...
      setSuccessMsg(`✓ Successfully registered access for "${emailInput}" (${role} privilege).`);
      setEmail('');
      setName('');
      setPassword('');
      setRole('User');
  ...
  const handleUpdateUser = async (targetEmail: string) => {
    ...
    try {
      ...
      setSuccessMsg(`✓ Successfully updated registration parameters for "${targetEmail}".`);
      setEditingEmail(null);
      fetchUsers();
      if (onRefreshData) onRefreshData();
    } catch (err: any) {
      setErrorMsg(err.message || 'Error occurred while editing user parameters.');
    }
  };
                        ...
                        <td className="py-3 px-2">
                          {isEditing ? (
                            <select
                              ...
                              className="bg-slate-950 border border-slate-800 rounded px-1.5 py-1 text-[10px] text-indigo-300 focus:outline-none focus:border-indigo-500 font-mono"
                            >
                              <option value="Admin">Admin</option>
                              <option value="Supervisor">Supervisor</option>
                        ...
                        <td className="py-3 px-2 font-mono text-[11px] text-indigo-300">
                          {isEditing ? (
                            <input
                              ...
                              className="bg-slate-950 border border-slate-800 rounded px-2 py-1 text-xs text-slate-200 w-32 focus:outline-none focus:border-indigo-500 font-mono"
                              required
                            />
                          ) : (
                            ...
                            {isEditing ? (
                              <>
                                <button
                                  ...
                                  className="p-1 px-1.5 rounded bg-emerald-500/10 text-emerald-400 hover:text-white hover:bg-emerald-600 font-bold transition-all cursor-pointer flex items-center gap-0.5"
                                  title="Save Changes"
                                >
                                ...
                                <button
                                  ...
                                  className="p-1 px-1.5 rounded bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700 transition-all cursor-pointer flex items-center gap-0.5"
                                  title="Cancel Editing"
                                >
                                  <X className="h-3 w-3" />
                                  <span className="text-[9px] font-mono">Cancel</span>
                                </button>
                              </>
                            ) : (
                              <>
                                {currentUser?.role === 'Admin' && (
                                  <button
                                    ...
                                    className="p-1 rounded bg-indigo-950/40 text-indigo-400 hover:text-white hover:bg-indigo-650/40 border border-indigo-900/30 transition-all cursor-pointer"
                                ...
                                <button
                                  ...
                                  className={`p-1 rounded border transition-all ${
                                    isSelf 
                                      ? 'text-slate-700 hover:bg-transparent border-transparent cursor-not-allowed opacity-30' 
                                      : 'text-red-400 hover:text-white bg-red-950/20 hover:bg-red-900 border-red-900/30 cursor-pointer'
                                  }`}
